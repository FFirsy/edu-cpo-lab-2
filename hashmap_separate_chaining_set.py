...
class HashMapSeparateChainingSet(Generic[T]):
    ...
    Uses structural sharing where possible to avoid unnecessary
    copying.
    """
    ...
    def __init__(
        ...
        self._buckets: Tuple[Tuple[T, ...], ...] = (
            tuple(tuple() for _ in range(capacity))
        )
        self._size = 0
    ...
    def _copy_with(
        ...
        """Create a new set, reusing self's fields where not
        overridden."""
        result = HashMapSeparateChainingSet.__new__(
    ...
    def _needs_resize(self, extra: int = 0) -> bool:
        return (
            (self._size + extra) / self._capacity
            > self._load_factor
        )

    def _resized(self) -> "HashMapSeparateChainingSet[T]":
        """Return a new set with capacity doubled and all elements
        rehashed."""
        new_capacity = self._capacity * 2
        new_buckets: List[Tuple[T, ...]] = [
            tuple() for _ in range(new_capacity)
        ]
    ...
    def _remove_from_bucket(
        ...
        """Recursively remove element from bucket; return new
        bucket."""
        if len(bucket) == 0:
            return tuple()
        if bucket[0] == element:
        ...
        return (
            (bucket[0],)
            + self._remove_from_bucket(element, bucket[1:])
        )
    ...
    def _bucket_to_list(
        ...
        return (
            [bucket[0]]
            + self._bucket_to_list(bucket[1:])
        )

    def _all_elements(
        self, buckets: Tuple[Tuple[T, ...], ...]
    ) -> List[T]:
        """Recursively collect all elements from all buckets."""
        if len(buckets) == 0:
            return []
        return (
            self._bucket_to_list(buckets[0])
            + self._all_elements(buckets[1:])
        )
    ...
    def _map_bucket(
        ...
        """Recursively map over a bucket; return list (may have
        duplicates after map)."""
        if len(bucket) == 0:
            return []
        return (
            [func(bucket[0])]
            + self._map_bucket(bucket[1:], func)
        )
    ...
    def _reduce_bucket(
        ...
        if len(bucket) == 0:
            return acc
        return self._reduce_bucket(
            bucket[1:], func, func(acc, bucket[0])
        )
    ...
    def cons(
        ...
        return ensured._copy_with(
            buckets=tuple(new_buckets),
            size=ensured._size + 1,
        )
    ...
    def remove(
        ...
        new_buckets[idx] = self._remove_from_bucket(
            element, bucket
        )
        return self._copy_with(
            buckets=tuple(new_buckets),
    ...
    def member(self, element: T) -> bool:
        idx = hash(element) % self._capacity
        return self._elem_in_bucket(
            element, self._buckets[idx]
        )
    ...
    def filter(
        ...
        """Return a new set with only elements satisfying
        predicate."""
        new_buckets: List[Tuple[T, ...]] = [
            self._filter_bucket(bucket, predicate)
            for bucket in self._buckets
        ]
        new_size = sum(len(b) for b in new_buckets)
        return self._copy_with(
            buckets=tuple(new_buckets), size=new_size
        )

    def map(
        self, func: Callable[[T], Any]
    ) -> "HashMapSeparateChainingSet[Any]":
        """Return a new set by applying func to each element
        (may collapse duplicates)."""
        result: List[Any] = []
        for bucket in self._buckets:
            result.extend(
                self._map_bucket(bucket, func)
            )
    ...
    def reduce(
        ...
        """Reduce all elements with func, starting from
        initial."""
        if self._size == 0 and initial is None:
            raise TypeError(
        ...
        if initial is not None:
            return self._reduce_all_buckets(
                buckets, 0, func, initial
            )

        # initial is None, use first element as accumulator
        for i, bucket in enumerate(buckets):
            if len(bucket) > 0:
                acc: Any = bucket[0]
                acc = self._reduce_bucket(
                    bucket[1:], func, acc
                )
                return self._reduce_remaining_buckets(
                    buckets, i + 1, func, acc
                )
        raise TypeError(
            "reduce of empty set with no initial value"
        )
    ...
    def _reduce_remaining_buckets(
        self,
        buckets: Tuple[Tuple[T, ...], ...],
        start_idx: int,
        func: Callable[[S, T], S],
        acc: S,
    ) -> S:
        """Reduce buckets starting from start_idx (skip
        already-processed buckets)."""
        if start_idx >= len(buckets):
            return acc
        acc = self._reduce_bucket(
            buckets[start_idx], func, acc
        )
        return self._reduce_remaining_buckets(
            buckets, start_idx + 1, func, acc
        )
    ...
    def _reduce_all_buckets(
        ...
        if idx >= len(buckets):
            return acc
        acc = self._reduce_bucket(
            buckets[idx], func, acc
        )
        return self._reduce_all_buckets(
            buckets, idx + 1, func, acc
        )

    def intersection(
        self, other: "HashMapSeparateChainingSet[T]"
    ) -> "HashMapSeparateChainingSet[T]":
        """Return a new set with elements present in both
        sets."""
        result = empty()
...
def cons(
    s: HashMapSeparateChainingSet[T],
    element: T,
) -> HashMapSeparateChainingSet[T]:
    """Function-style cons: return a new set with element
    added."""
...
def remove(
    ...
    """Function-style remove: return a new set with element
    removed."""
    return s.remove(element)


def member(
    s: HashMapSeparateChainingSet[T], element: T
) -> bool:
...
def from_list(
    ...
    """Function-style from_list: build a set from a Python
    iterable."""
...
def intersection(
    s1: HashMapSeparateChainingSet[T],
    s2: HashMapSeparateChainingSet[T],
) -> HashMapSeparateChainingSet[T]:
